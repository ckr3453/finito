...
- home_widget 패키지 통합
- 플랫폼별 네이티브 위젯 구현 (iOS, Android, macOS)
- 위젯 ↔ 앱 데이터 동기화
- WidgetService + HomeWidgetClient 추상화
- Deep Link URI Scheme + GoRouter 연동
- App Lifecycle 위젯 갱신 연동

## Phase 5: 알림 ✅

로컬 리마인더 + FCM 토큰 관리.

- flutter_local_notifications + timezone (timezone-aware 스케줄링)
- firebase_messaging 토큰 취득/갱신 구조
- TaskEntity.reminderTime 필드 + Drift v3 마이그레이션
- NotificationService + LocalNotificationClient 추상화 (TDD)
- FcmClient + FcmService 추상화 (TDD)
- TaskEditor 리마인더 DateTimePicker, TaskDetail 표시
- Settings 알림 권한 섹션
- 자동 재스케줄 (reminderAutoRescheduleProvider)
